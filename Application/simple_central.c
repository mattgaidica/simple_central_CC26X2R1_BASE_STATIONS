...
#define TIME_SERVICE_UUID		0xEFFE // explicit in SimpleCentral_processGATTDiscEvent()
#define TIME_CHAR_UUID			0xEFFF
...
//		Display_printf(dispHandle, SC_ROW_ANY_CONN, 0, "Insufficient Memory");
	...
	case ESLO_TIME_FOUND: {
		uint8_t i;
		for (i = 0; i < 5; i++) {
			GPIO_write(CONFIG_GPIO_LEDG, i % 2);
			GPIO_write(CONFIG_GPIO_LEDR, (i + 1) % 2);
			Task_sleep(5000);
		}
		GPIO_write(CONFIG_GPIO_LEDG, 0x00);
		GPIO_write(CONFIG_GPIO_LEDR, 0x00);
...
//						pMsg->msg.readRsp.pValue[0]);
				// Matt
				memcpy(&curTime, pMsg->msg.readRsp.pValue, sizeof(curTime));
			}
...
static void SimpleCentral_processGATTDiscEvent(gattMsgEvent_t *pMsg) {
	...
	if (discState == BLE_DISC_STATE_MTU) {
		...
		if (pMsg->method == ATT_EXCHANGE_MTU_RSP) {
			uint8_t uuid[ATT_BT_UUID_SIZE] = { LO_UINT16(TIME_SERVICE_UUID),
		...
		if (((pMsg->method == ATT_FIND_BY_TYPE_VALUE_RSP)
			...
			if (svcStartHdl != 0) {
				...
				req.type.uuid[0] = LO_UINT16(TIME_CHAR_UUID);
				req.type.uuid[1] = HI_UINT16(TIME_CHAR_UUID);
...
//      Display_printf(dispHandle, SC_ROW_CUR_CONN, 0, "Simple Svc Found");
		...
		discState = BLE_DISC_STATE_READ;

	} else if (discState == BLE_DISC_STATE_READ) {
		if ((pMsg->method == ATT_READ_BY_TYPE_RSP
				&& pMsg->hdr.status == bleProcedureComplete)
				|| (pMsg->method == ATT_ERROR_RSP)) {
			// Matt
